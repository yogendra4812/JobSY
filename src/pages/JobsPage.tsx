  ...
  try {
    const res = await fetch(
      `${apiBase}/apply-job?user_id=${encodeURIComponent(user.id)}&job_id=${encodeURIComponent(pendingJob.id)}`,
      {
        ...
        body: JSON.stringify({ appliedAt: now }),
      }
    );
    if (res.ok) {
      // 1) Refresh the "Applied" tab
      await fetchAppliedJobs();

      // 2) Unconditionally re‐run whatever recommendation query is active
      await refreshRecommended();
    }
  } catch (err) {
    console.error('Error confirming apply:', err);
  } finally {
    // Clean up the modal/session & reset back to "Recommended"
